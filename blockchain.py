...
class Blockchain:
    def __init__(self, _types, _parents, _winners):
        #
        ...
        ### ASSERTIONS ###
        assert self.winners.shape[0] == self.parents.shape[0], "arrays \'parents\' and \'winners\' must be of same length"
        for t in range(self.horizon):
            assert self.winners[t] < self.types.shape[0], "only miners who exist can win"
            assert self.parents[t] < t+1, "the parent of a block must have a strictly lower index"
    ...
    def __assignMiners(self):
        ...
        n = np.sum(self.types)
        p, q, r = self.types[0], self.types[1], self.types[2]

        miners = np.array([])
        for i in range(n):
            miners = np.append(miners, Miner(self.horizon, np.nan))
    ...
    def __generateChain(self):
        ...
        for t in range(self.horizon + 1):  #fill a chain with default Blocks, we have 'T+1' blocks: that is 'T' blocks mined plus the genesis block
            if t == 0:
                sequence = np.append(sequence, Block(0, np.nan, np.nan))
                sequence[0].genesis = True
            else:
                sequence = np.append(sequence, Block(t,
                                                     self.parents[t-1],
                                                     self.winners[t-1]))
        return sequence
    ...
    def getLongestChainStrategy(self, _i, _t):
        ...
        #'_t' is the stage at which we want to compute the strategy
        #FIXME: we want to have ALL viable strategies returned.
        #
...
class ExtendedBlockchain(Blockchain):
    def __init__(self, _B, _i, _t):
        ...
        Blockchain.__init__(self, types, parents, winners) #build
