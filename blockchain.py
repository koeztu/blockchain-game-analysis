...
class Blockchain:
    ...
    def __assignMiners(self):
        ...
        #assign conservative miners
        for i in range(p):
            miners[i].number = i
            miners[i].conservative = True

        #assign longestchain miners
    ...
    def to_csv(self, _filename):
        ...
        row = ["types"]
        row.append("conservative")
        row.append(self.types[0])
        row.append("longest-chain")
        row.append(self.types[1])
        row.append("naive")
        row.append(self.types[2])
        writer.writerow(row) #write one row
